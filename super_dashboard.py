...
# ✨ 取代 yfinance，全面改用 FinMind 抓 K 線，徹底免疫 Yahoo 封鎖
...
with col_ctrl1:
    raw_input = st.text_input("搜尋", "8091", label_visibility="collapsed", placeholder="輸入代號或中文名稱 (如: 8091 或 翔名)")
with col_ctrl2:
    analyze_btn = st.button("🔥 啟動全板面解析", use_container_width=True)
with col_ctrl3:
    st.markdown("<div style='margin-top: 8px; font-size: 0.9em; color:gray;'>※ 搭載 FinMind VIP 純血雙引擎，完全免疫 Yahoo 封鎖</div>", unsafe_allow_html=True)
...
if analyze_btn or raw_input:
    ...
    with st.spinner(f'鎖定目標 [{display_title}] ... 啟動戰情解析中...'):
        try:
            ...
            if len(df_full) < 20:
                st.error(f"🚨 無法取得 [{display_title}] 的有效報價資料。請確認代號正確，或該股可能暫停交易。")
                st.stop()
                
            df_full.index = df_full.index.tz_localize(None)
            
            # FinMind 的成交量單位是「股」，除以 1000 變成「張」
